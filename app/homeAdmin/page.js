...
async function getMataKuliah(){
    const res = await fetch(`${process.env.POSTGRES_URL}/api/homeAdmin`,{cache:'no-store'})
    const result = await res.json();
    return result
}

async function getNamaMataKuliah(){
    const res = await fetch(`${process.env.POSTGRES_URL}/api/homeAdmin/ujianAdd`,{cache:'no-store'})
    const result = await res.json();
    return result
}
