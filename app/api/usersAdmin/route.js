...
export async function POST (req, res){
  try {
    ...
    const client = await pool.connect()
    const result = await client.query(`insert into "user" (email, fullname, role) values ('${request.email}', '${request.fullname}', '${request.role}')`)
    client.release()
...
export async function DELETE(req, res){
  try {
      ...
      if(request.idAdmin!=null){
        const client = await pool.connect();
      const result = await client.query(`DELETE FROM "user" where "idUser" = '${request.idAdmin}'`);
      // res.status(200).json(result);
      // return new Response(JSON.stringify(result.rows));
      client.release()
      return NextResponse.json(result)
