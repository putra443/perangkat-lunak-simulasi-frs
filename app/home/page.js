      ...
      <div className="flex min-h-screen flex-col items-center px-20 text-center bg-cover bg-center h-screen" style={{backgroundImage: `url(${bg.src})`}}>
        ...
        <div className='flex flex-col px-20 w-screen h-screen bg-gradient-to-br from-sky-500'>
          ...
          <div className='flex text-xl mt-2 p-5 space-x-10'>
            {session?.user?.role=="Admin / Mahasiswa" ? (<a href={`/simulasi/${session?.user?.role.substring(8,session?.user?.role.length)}/${session?.user?.email}/${session?.user?.id}`}    className='btn border-none bg-sky-700 hover:bg-sky-900 text-lg px-4 py-2 rounded-3xl  text-white '>Simulasi FRS</a>) 
            : 
            (<a href={`/simulasi/${session?.user?.role}/${session?.user?.email}`}    className='btn border-none hover:bg-sky-900 text-lg px-4 py-2 rounded-3xl bg-sky-700 text-white'>Simulasi FRS</a>)}
